...
namespace Aspect.UI.Web
{
    ...
    {    
        ...
        protected System.Web.UI.HtmlControls.HtmlAnchor ShowAttachedFiles;
        ...
        protected void InitialTopPageBar()
        {
            ...
            this.PopupIframeInitializationString(ShowAttachedFiles, "Присоединённые документы",
                string.Format("Popup/AttachedFiles.aspx?pid='+selectedProductID+'",
                hiddenSelectedProducts.Value == null ? "" : hiddenSelectedProducts.Value.ToString()), 700, 500);            
            ...
            if (data != null && data.Tables.Count > 0)
            {
                ...
                foreach (DataRow row in data.Tables[2].Rows)
                {
                    ...
                    foreach (GridColumn column in columns)
                    {
                        ...
                        {
                            if (row[column.DataItem].GetType() == typeof(bool))
                            {
                                dataRow.Add(row[column.DataItem]);
                            }
                            else
                            {
